...
export class PortalLandingComponent implements OnInit {
  ...
  // Schools list for modern interactive matrix
  schools: SchoolInfo[] = [
  ...
  // Detailed Guidelines
  guidelines: { id: number; title: string; detail: string; link?: string; linkText?: string }[] = [
  ...
  get filteredSchools(): SchoolInfo[] {
    ...
    return this.schools.filter(s => s.syllabus === this.schoolFilter);
  }
  ...
  loadPortalLinks() {
    ...
    this.portalService.getLandingData().subscribe({
      ...
      error: () => {
        this.isLoadingLinks = false;
        // Premium default fallback links
        this.admissionLinks = [
